...
def _esc(text) -> str:
    ...
    FIX #13: Prevents XSS via user/report-derived content injected
    into unsafe_allow_html blocks.
    """
    if text is None:
        return ""
...
def render_disclaimer():
    ...
    st.markdown(
        ...
        ⚕️ <strong>Medical Disclaimer:</strong> This tool is for <strong>educational and informational purposes only</strong>.
...
def render_mode_badge(mode: str):
    ...
    if mode == "patient":
        st.markdown(
            '<span class="mode-badge mode-badge-patient">🧑‍⚕️ Patient Mode</span>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<span class="mode-badge mode-badge-community">🏥 Community Intelligence Mode</span>',
            unsafe_allow_html=True,
        )
...
def render_risk_card(risk_card: dict):
    ...
    FIX #13: All dynamic values are HTML-escaped.
    """
    ...
    st.markdown(
        f"""<div class="{css_class} animate-in">
            ...
            <p style="margin-top: 8px; font-size: 0.85rem; opacity: 0.7;">
                ...
                ⚠️ {_esc(risk_card['abnormal_count'])} abnormal,
...
def render_lab_value_card(fv: FlaggedValue):
    ...
    FIX #13: All dynamic values are HTML-escaped.
    FIX #14: Uses `is not None` instead of falsy check on reference_low
    so a legitimate 0 value is displayed correctly.
    """
...
def render_lab_values_grid(flagged_values: list[FlaggedValue]):
    ...
    # Filter out UNKNOWN flags for cleaner display
...
def render_source_evidence(source_chunks: list[str], source_metadata: list[dict] = None):
    """Render source evidence chunks (Responsible AI).

    FIX #13: All dynamic content is HTML-escaped.
    """
    ...
    with st.expander("📄 Source Evidence (Retrieved Report Sections)", expanded=False):
        st.markdown(
            '<div class="source-label">These sections from the original report were used to generate the answer:</div>',
            unsafe_allow_html=True,
        )
